...
if (-not (Test-Path $dataDir)) {
  throw "Local database cluster not found. Run npm.cmd run db:local:init --workspace backend first."
}
...
if ($LASTEXITCODE -eq 0) {
  Write-Host "Local SyncDoc PostgreSQL is already running on port $port."
  exit 0
}
